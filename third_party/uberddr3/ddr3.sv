...
`endif
    ...
    function string sanitize_string(string in_str);
        string out_str;
        byte c;
        int i;

        out_str = in_str; // Copy input to output

        for (i = 0; i < out_str.len(); i++) begin
            c = out_str[i];
            ...
            if ( ! (
                ...
                c == "." ||                //-- Dot
                c == "-"                   //-- Dash
                //-- Add (c == "/") here if you want to preserve directory paths
            )) begin
                out_str[i] = "_"; //-- Replace bad char with underscore
            end
        end

        if (out_str.len() > 64) begin
            out_str = out_str.substr(out_str.len() - 64, out_str.len() - 1);
        end
...
`ifdef MAX_MEM
    ...
    function integer open_bank_file( input integer bank );
        integer fd;
        reg [2048:1] filename;
        // XXX(fmil): Used to clean up module hierarchy to derive a "clean"
        // file name to be used here.
        string raw_hier, clean_hier;
        begin
            raw_hier = $sformatf("%m");
            clean_hier = sanitize_string(raw_hier);
            $sformat( filename, "%0s/%0s.%0d", tmp_model_dir, clean_hier, bank );
            $display("opening model data file: %0s", filename);
